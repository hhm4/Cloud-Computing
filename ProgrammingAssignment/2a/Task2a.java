...
public class Task2a {
	...
	public static class Reducer2 extends Reducer<Text,Text,Text,Text>{
		public void reduce(Text key,Iterable<Text> values,Context context) throws IOException, InterruptedException{
			int maxVal=0;
			String dominantWord=new String();
			for (Text val:values){
				int x=Integer.parseInt(val.toString().substring(1, val.toString().length()));
				if (x>maxVal){
					maxVal=Integer.parseInt(val.toString().substring(1, val.toString().length()));
					dominantWord=val.toString().substring(0,1);
				}
				
			}
			context.write(key, new Text(dominantWord.toString()));
		}
	}
	...
	public static class Reducer3 extends Reducer<Text,LongWritable,Text,LongWritable>{
		public void reduce(Text key,Iterable<LongWritable> values,Context context) throws IOException, InterruptedException{
			...
			switch(key.toString()){
			case "s":
				context.write(new Text("Word: Sports =>  No of States :"), new LongWritable(count));
				break;
			case "e":
				context.write(new Text("Word: Education =>  No of States :"), new LongWritable(count));
				break;
			case "p":
				context.write(new Text("Word: Politics =>  No of States :"), new LongWritable(count));
				break;
			case "a":
				context.write(new Text("Word: Agriculture =>  No of States :"), new LongWritable(count));
				break;
			}
...
public static void main(String []args)throws Exception{
		...
		job1.setJarByClass(Task2a.class); 
		...
		job2.setJarByClass(Task2a.class);
		FileInputFormat.addInputPath(job2, new Path("/job1Output"));//output1 & input2
		FileOutputFormat.setOutputPath(job2,new Path("/job2Output"));//output2 or input3
		...
		job3.setJarByClass(Task2a.class);
		FileInputFormat.addInputPath(job3, new Path("/job2Output"));//output2 & input3
